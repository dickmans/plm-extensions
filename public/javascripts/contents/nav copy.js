...
function insertWorkspaceItems(wsId, params) {
    ...
    let filters             = [];       // Define optional filters by providing an array of key pairs (fieldId, comparator and value)
    ...
    if(!isBlank(params.filters)           ) {            filters = params.filters;             bulk = true; }
    ...
    if(bulk) {
        ...
        if(sortBy === '') sortBy = 'DESCRIPTOR';

        url             = '/plm/search';
        payload.sort    = [sortBy];
        payload.fields  = [sortBy];
        payload.filter = [];
        
        if(   fieldIdImage !== '') payload.fields.push(fieldIdImage);
        if(   fieldIdTitle !== '') payload.fields.push(fieldIdTitle);
        if(fieldIdSubtitle !== '') payload.fields.push(fieldIdSubtitle);
        
             if(groupBy.toLowerCase() === 'status')          groupBy = 'WF_CURRENT_STATE';
        else if(groupBy.toLowerCase() === 'state')           groupBy = 'WF_CURRENT_STATE';
        else if(groupBy.toLowerCase() === 'workflow-status') groupBy = 'WF_CURRENT_STATE';

        if( sortBy !== '') if(payload.fields.indexOf(sortBy) < 0) payload.fields.push(sortBy);
        if(groupBy !== '') if(payload.fields.indexOf(sortBy) < 0) payload.fields.push(groupBy);

        for(let fieldId of fieldIdsDetails)    payload.fields.push(fieldId);
        for(let fieldId of fieldIdsAttributes) payload.fields.push(fieldId);

        for(let filter of filters) {

            let filterType       = 0;
            let filterComparator = 5;
            ...
            if(payload.fields.indexOf(filter.field) < 0) payload.fields.push(filter.field);
    ...
    $.get(url, payload, function(response) {
        ...
        if(bulk) {
            ...
            // for(item of response.data.row) {
            //     item.sortKey = getSearchResultFieldValue(item, sortBy);
            // }
            ...
            if(!isBlank(groupBy)) {
                for(item of response.data.row) {
                    item.groupKey = getSearchResultFieldValue(item, groupBy);
                    // if(groupBy === 'status') {
                ...
                sortArray(response.data.row, 'groupKey', 'string', 'ascending');
            }
            ...
            for(item of response.data.row) {
                ...
                let image    = (fieldIdImage === '') ? null : getSearchResultFieldValue(item, fieldIdImage);
                let title    = getSearchResultFieldValue(item, fieldIdTitle);
                let subtitle = (fieldIdSubtitle === '') ? null : getSearchResultFieldValue(item, fieldIdSubtitle);
                let details  = [];

                console.log(image);

                let elemTile = genTile('/api/v3/workspaces/' + response.params.wsId + '/items/' + item.dmsId, '', image, icon, title, subtitle);
                ...
                for(let fieldidDetail of fieldIdsDetails) {
                    ...
                    details.push([
                        fieldidDetail[0],
                        getSearchResultFieldValue(item, fieldidDetail[1]),
                        fieldidDetail[2]
                    ]);
                ...
                console.log(fieldIdsAttributes);

                for(let fieldAttribute of fieldIdsAttributes) {

                    console.log(getSearchResultFieldValue(item, fieldAttribute));

                    elemTile.attr('data-' + fieldAttribute.toLowerCase(), getSearchResultFieldValue(item, fieldAttribute));
