...
let workspaces        = [];
let paramsDetails     = {};
let completionEvents  = { landing : false };
let mappingTablesData = [];    // Stores relevant data of mapping tables managed in M/E Alignment application
...
// TODO : Add Nodes
// TODO : Handle BOM Quantity for ME items in MEBOM
// TODO : Color codes
// TODO : Color Bar
// TODO : Insert sub nodes
// TODO : Number / Title as columsn in Process BOM
// TODO : Actions for assemblies
// TODO : Add From Library
// TODO : Cross Highlight
// TODO : Quantity Comparison
// TODO : Make / Buy Handling
...
$(document).ready(function() {
    ...
    paramsDetails             = config.appEditor.panels.itemDetails;
...
// Set UI labels and events
function setApplicationLabels() {

    $('#header-title').html(config.appEditor.labels.appTitle || 'Process BOM Editor');
...
function setLandingPage() {
    ...
    let paramsLandingViews                = config.appEditor.panels.landingViews;
        paramsLandingViews.id             = 'views';
        paramsLandingViews.onDblClickItem = function(elemClicked) { openItem(elemClicked.attr('data-link')); }
    
    let paramsLandingSearch               = config.appEditor.panels.landingSearch;
...
function openItem(link) {
    ...
    $.get('/plm/details', { link : link }, function(response) {
        ...
        urlParameters.title = response.data.title;

        $('body').addClass('screen-main').removeClass('screen-landing');
        $('#header-subtitle').html(urlParameters.title).removeClass('hidden');
        $('#overlay').hide();        

        links.mcad = getSectionFieldValue(response.data.sections, config.wsContext.fieldIds.mcad , '', 'link');
        links.ecad = getSectionFieldValue(response.data.sections, config.wsContext.fieldIds.ecad , '', 'link');
        links.pbom = getSectionFieldValue(response.data.sections, config.wsContext.fieldIds.pbom , '', 'link');

        insertViewer(links.mcad, { 
            cacheInstances     : true,
            cacheBoundingBoxes : true,
            features           : config.appEditor.viewerFeatures,
            // afterCompletion    : function(id) { afterViewerCompletion(id); }
        });

        let paramsMEBOM = config.appEditor.panels.insertMEBOM;
            paramsMEBOM.id                 = 'mebom';
            paramsMEBOM.dragable           = false;
            paramsMEBOM.dropable           = false;
            paramsMEBOM.afterCompletion    = function(id, data)    { afterMEBOMCompletion(id, data); };
            paramsMEBOM.onClickItem        = function(elemClicked) { onClickMEItem(elemClicked); };
            paramsMEBOM.additionalRequests = [ $.get('/plm/grid', { link : links.ecad} ) ];


        for(let workspace of workspaces) {
            workspace.link = getSectionFieldValue(response.data.sections, workspace.fieldId, '', 'link');
            paramsMEBOM.additionalRequests.push($.get('/plm/grid', { link : workspace.link }));
        }

        insertBOM(links.mcad, paramsMEBOM);

        let paramsPBOM = config.appEditor.panels.insertPBOM;
            paramsPBOM.id                 = 'pbom';
            paramsPBOM.dragable           = false;
            paramsPBOM.dropable           = false;
            paramsPBOM.editable           = true;
            paramsPBOM.afterCompletion    = function(id, data)    { afterPBOMCompletion(id, data); };
            paramsPBOM.additionalRequests = [ ];

        insertBOM(links.pbom, paramsPBOM);
...
// M/E BOM Tree
function afterMEBOMCompletion(id, data) {

    let mappedItems    = [];
    let ecadItems      = [];
    let elemTBody      = $('#mebom-tbody');
    let alignmentTypes = [];

    for(let workspace of workspaces) alignmentTypes.push(workspace.label);


    $('#mebom-tbody .tree-item').each(function() { $(this).addClass('mcad-item'); });

    getMappingTablesData(data.dataAdditional);

    console.log(mappingTablesData);

    for(let eItem of data.dataAdditional[0].data) {

        let item = {
            link        : getGridRowValue(eItem, config.ecadBOM.fieldIds.item       , '', 'link'),
            partNumber  : getGridRowValue(eItem, config.ecadBOM.fieldIds.partNumber , ''),
            title       : getGridRowValue(eItem, config.ecadBOM.fieldIds.title      , ''),
            className   : getGridRowValue(eItem, config.ecadBOM.fieldIds.class      , '', 'title'),
            linkMapping : getGridRowValue(eItem, config.ecadBOM.fieldIds.linkMapping, '')
        }

        let isAlignmentType = alignmentTypes.includes(item.className);
    ...
    sortArray(ecadItems, 'className');

    console.log(ecadItems);
    console.log(mappedItems);
    console.log(mappingTablesData);
    
    insertMappingTableEntries();
    
    let prevClassName = '';

    for(let ecadItem of ecadItems) {

        if(prevClassName !== ecadItem.class) {

            let elemRowGroup = genTreeRow('mebom', {
                title       : ecadItem.className,
                hasChildren : true,
                level       : 1
            });

            elemRowGroup.appendTo(elemTBody);

        }

            let elemRowItem = genTreeRow('mebom', ecadItem);
                elemRowItem.appendTo(elemTBody);        
                elemRowItem.addClass('class-cable');

        prevClassName = ecadItem.class;

    }


    
    setLeafItemsDragable();
    treeInsertIconColumn('mebom');

    if(settings['mebom'].collapseContents) collapseAllNodes('mebom');

}
function getMappingTablesData(additionalData) {

    mappingTablesData = [];

    for(let index = 1; index < additionalData.length; index++) {

        let mappingTable = additionalData[index].data;

        for(let mappingRow of mappingTable) {
            let mapping = {
                link        : getGridRowLink(mappingRow),
                tag         : getGridRowValue(mappingRow, config.mappingTables.fieldIds.tag, ''),
                partNumber  : getGridRowValue(mappingRow, config.mappingTables.fieldIds.partNumber, ''),
                linkECAD    : getGridRowValue(mappingRow, config.mappingTables.fieldIds.linkECAD, '', 'link'),
                title       : getGridRowValue(mappingRow, config.mappingTables.fieldIds.linkECAD, '', 'title'),
                locMCAD     : getGridRowValue(mappingRow, config.mappingTables.fieldIds.locMCAD, ''),
                locViewer   : getGridRowValue(mappingRow, config.mappingTables.fieldIds.locViewer, ''),
                linkECADBOM : getGridRowValue(mappingRow, config.mappingTables.fieldIds.linkECADBOM, ''),
                index       : mappingTablesData.length
            }
            mappingTablesData.push(mapping);
        }

    }

}
function insertMappingTableEntries() {

    $('#mebom-tbody .content-item').each(function() {

        let elemTreeItem = $(this);
        let treeItemPath = elemTreeItem.attr('data-number-path');
        let toggleNode   = false;

        for(let mapping of mappingTablesData) {

            if(mapping.locMCAD === treeItemPath) {

                mapping.level = Number(elemTreeItem.attr('data-level')) + 1;
                toggleNode    = true;

                let elemTreeRow = genTreeRow('mebom', mapping);

                    elemTreeRow.insertAfter(elemTreeItem);
                    elemTreeRow.addClass('ecad-item');
                    elemTreeRow.attr('data-mapping-index', mapping.index);

                if(mapping.linkECADBOM === '') elemTreeRow.addClass('missing');
                
            }

        }

        if(toggleNode) treeToggleItemNodeClass(elemTreeItem);

    });

}
function setLeafItemsDragable() {

    $('#mebom-tbody .content-item.leaf').each(function() {
    
        let elemRow = $(this);

        elemRow.attr('draggable'  , 'true');
        elemRow.attr('ondragstart', function() {});
        elemRow.attr('ondragend'  , function() {});
...
// Process BOM Tree
function afterPBOMCompletion(id, data) {

    let elemControls = $('#pbom-actions');
...
// M/E BOM Functionality
function onClickMEItem(elemClicked){

    console.log('onClickMEItem');

    let partNumber = elemClicked.attr('data-part-number')   || '';
    let root       = elemClicked.attr('data-root-link')     || '';
    let path       = elemClicked.attr('data-number-path')   || '';
    let mapIndex   = elemClicked.attr('data-mapping-index') || '';
    // console.log(Number(mapIndex));
    let instance   = (mapIndex === '') ? '' : mappingTablesData[Number(mapIndex)].locViewer;

    // console.log(elemClicked.attr('data-mapping-index'));
    // console.log(mapIndex);
    // console.log(instance);
    // console.log(typeof mapIndex);
    // console.log(mappings[mapIndex]);

console.log(partNumber, root, path, instance);

    crossHighlight(partNumber, root, path, instance);

}
function crossHighlight(partNumber, root, path, instance) {

    $('*').removeClass('selected');

    // viewerHighlightInstances(partNumber, null, path, {});

    if(instance === '') viewerSelectModels([partNumber]);
    else {
        console.log(instance);
        viewerHighlightInstances(partNumber, null, [instance], {});
    }
...
// Process BOM Functionality
function insertNewPBOMNode(elemParent) {

    console.log('insertNewPBOMNode');

    let elemRow = genTreeRow('pbom', {
        hasChildren : true,
        title : 'new',
        level : 1
    });

    elemRow.addClass('process');
    elemRow.attr('ondragenter', 'onTreeDragEnter(event)');
    elemRow.attr('ondragover' , 'onTreeDragOver(event)' );
    elemRow.attr('ondragleave', 'onTreeDragLeave(event)');
    elemRow.attr('ondrop'     , 'onTreeDrop(event)'     );    


    console.log(elemParent);
    console.log($('#pbom-tbody').length);

    if(elemParent === null) {

        elemRow.appendTo($('#pbom-tbody'));

    }

    $('#pbom-no-data').addClass('hidden');
    $('#pbom-content').show();
...
function onTreeDrop(e) {
    ...
    const level       = Number(elemTarget.attr('data-level')) + 1;

    console.log(level);
    console.log(elemTarget.length);
    console.log(elemDragged.attr('data-title'));

    let elemTreeRow = genTreeRow('pbom', {
        title : elemDragged.attr('data-title'),
        level : level
    });

    elemTreeRow.insertAfter(elemTarget);
